locals {
  ...
  # Full image path from Docker Hub
  full_image = "${var.dockerhub_image}:${var.image_tag}"
}

resource "kubernetes_namespace" "kubeflake" {
  metadata {
    name = var.namespace
  }
}
...
resource "kubernetes_service" "keygen_headless" {
  metadata {
    ...
    namespace = kubernetes_namespace.kubeflake.metadata[0].name
    labels = {
  ...
  depends_on = [
    kubernetes_namespace.kubeflake,
  ]
}
